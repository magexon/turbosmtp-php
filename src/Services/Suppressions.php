...
class Suppressions extends TurboSMTPService {
    ...
    private function restrictions(array $restrictions): array
    {
        return array_map(function(SuppressionsRestriction $criteria) {
            ...
            $byMapping = [
                SuppresionsRestrictionFilterBy::SENDER() => SuppressionRestrictBy::SENDER,
                SuppresionsRestrictionFilterBy::RECIPIENT() => SuppressionRestrictBy::RECIPIENT,
                SuppresionsRestrictionFilterBy::REASON() => SuppressionRestrictBy::REASON,
                SuppresionsRestrictionFilterBy::SUBJECT() => SuppressionRestrictBy::SUBJECT,
            ];
        
            $t->setBy($byMapping[$criteria->getBy()->getValue()]);
        
            // Map the enum value for 'operator'
            $operatorMapping = [
                SuppressionsRestrictionOperator::INCLUDE() => SuppressionOperator::_INCLUDE,
                SuppressionsRestrictionOperator::EXCLUDE() => SuppressionOperator::EXCLUDE,
            ];
        
            $t->setOperator($operatorMapping[$criteria->getOperator()->getValue()]);
    ...
    private function setOrderByFromSuppressionsOrderBy(SuppressionsOrderByModel $orderBy): string {
        ...
        if (!array_key_exists($orderBy->getValue(), $orderByMapping)) {
            throw new \InvalidArgumentException("Invalid SuppressionsOrderBy value");
        }
    
        return $orderByMapping[$orderBy->getValue()];
    }
